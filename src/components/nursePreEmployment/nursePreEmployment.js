...
function NursePreEmployment() {
  ...
  const handleCloseModal = (e) => {
    if (e.target.classList.contains("npe-modal-overlay")) {
      closeDetailsModal();
    }
  };
  ...
  // Organized field groups for details modal
  const fieldGroups = {
    ...
    "Certificate Details": ["reason_for"],
    "Health Metrics": [
      "height",
      "weight",
      "bmi",
  ...
  return (
    <div className="npe-container">
      ...
      {isDetailsOpen && selectedLaborer && (
        <div className="npe-modal-overlay" onClick={handleCloseModal}>
          <div className="npe-modal-content" onClick={(e) => e.stopPropagation()}>
            <h3>Details for {selectedLaborer.name}</h3>
            <button className="npe-close-btn" onClick={closeDetailsModal}>
              &times;
            </button>
            <table className="npe-record-table">
              <tbody>
                {Object.entries(fieldGroups).map(([groupName, fields]) => (
                  <React.Fragment key={groupName}>
                    <tr className="npe-group-header-row">
                      <td colSpan="2" className="npe-group-header">
                        <h4>{groupName}</h4>
                      </td>
                    </tr>
                    {fields.map((key) => {
                      const excludeFields = ["id", "site_id", "created_at", "active", "laborer_id"];
                      if (excludeFields.includes(key)) return null;
                      let displayValue = selectedLaborer[key] || "N/A";
                      if (key === "identification_mark1" || key === "identification_mark2") {
                        try {
                          const parsed = typeof selectedLaborer[key] === 'string' 
                            ? JSON.parse(selectedLaborer[key]) 
                            : (Array.isArray(selectedLaborer[key]) ? selectedLaborer[key] : []);
                          displayValue = Array.isArray(parsed) && parsed.length > 0 ? parsed.flat().join(', ') : "N/A";
                        } catch (e) {
                          displayValue = "N/A";
                        }
                      }
                      ...
                      return (
                        <tr key={key} className="npe-field-row">
                          <td className="npe-field-label-cell">{label}:</td>
                          <td className="npe-field-value-cell">
            ...
            <div className="npe-modal-actions">
              <button className="npe-cancel-btn" onClick={closeDetailsModal}>
                Close
              </button>
            </div>
          </div>
