...
export default function MedPanel() {
  ...
  const api = axios.create({
    baseURL: "https://healthcop-website-backend-1.onrender.com",
    timeout: 10000,
  });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const res = await api.post("/api/attendance/login", {
        user_id: userId,
        site_id: siteId,
        site_name: siteName,
  ...
  const handleLogout = async (sessionId) => {
    ...
    try {
      ...
      const res = await api.put(`/api/attendance/logout/${sessionId}`, {
